...
void FW::BatchSplitBVHBuilder::initBBArrays(S32 maxN, FW::Scene* scene, FW::BVH& bvh)
{
    ...
    m_outIdxBest = m_segIdx + maxN;
    m_outIdxNew = m_outIdxBest + maxN;
    m_outCostBest = (F32*)(m_outIdxNew + maxN);
    m_outCostNew = m_outCostBest + maxN;
    m_outStratCount = (S32*)(m_outCostNew + maxN);
...
void FW::BatchSplitBVHBuilder::doGeneration(S32& N, S32& nSegments, S32 level)
{
    ...
    S32*  gamma = m_gamma;
    S32*  segIdx = m_segIdx;
    S32*  outIdxBest = m_outIdxBest;
    S32*  outIdxNew = m_outIdxNew;
    F32*  outCostBest = m_outCostBest;
    F32*  outCostNew = m_outCostNew;
    S32*  outStratCount = m_outStratCount;
    ...
    for (int dim = 0; dim < 3; dim++) {
        ...
        // OPT: segIdx is unneeded; should use a discard_iterator to get rid of rightIdx output, but was getting errors.
        auto outValues = thrust::make_zip_iterator(thrust::make_tuple(dim == 0 ? outCostBest : outCostNew, dim == 0 ? outIdxBest : outIdxNew, segIdx));
        ...
        if (dim == 0) {
            // Compute the count
            auto IIZipIt = thrust::make_zip_iterator(thrust::make_tuple(outIdxBest, segIdx)); // These currently contain left and right counts per segment

            thrust::transform_exclusive_scan(thrust::device,
                IIZipIt, IIZipIt + nSegments,
                //thrust::make_zip_iterator(thrust::make_tuple(outIdxBest + nSegments, segIdx + nSegments)),
                outStratCount, [] BHD(auto v) { return thrust::get<0>(v) + thrust::get<1>(v); },
            ...
            thrust::for_each_n(thrust::device, thrust::counting_iterator<S32>((S32)0), nSegments, [=] BHD(S32 i) {
                S32 ind = stratNumMask & outStratCount[i];
                S32 leafN = outIdxBest[i] + segIdx[i];
                F32 leafSAH = FW_F32_MAX;
                if (leafN <= minLeafSize)
                    leafSAH = FW_F32_MIN;
                else if (leafN <= maxLeafSize) {
                    AABB bounds = rightBounds[ind];
                    leafSAH = bounds.area() * getTriangleCost(leafN);
                }

                if (leafSAH < outCostBest[i]) {
                    outCostBest[i] = leafSAH;
                    outStratCount[i] = stratLeaf | ind;
                }
            });
        }
        else {
            ...
            thrust::for_each_n(thrust::device, thrust::counting_iterator<S32>((S32)0), nSegments, [=] BHD(S32 i) {
                if (outCostNew[i] < outCostBest[i]) {
                    outCostBest[i] = outCostNew[i];
                    outIdxBest[i] = outIdxNew[i];
                    outStratCount[i] = thisStrategy | (stratNumMask & outStratCount[i]);
                }
            });
        }
        ...
        if (level == 62) {
            ...
            for (int i = 0; i < nSegments; i++) {
                printf("%d 0x%x %d %f\n", i, (U32)outStratCount[i] >> stratBitOffset, stratNumMask & outStratCount[i], outCostBest[i]);
            }
        }
    }
    ...
    // Make segIdx be the per-reference index into out*
    ...
    // OPT: Think about setting up strats so I can sort by it; maybe this would give good spans for doing separate algorithms in next pass; maybe lets us keep a sorted array per dim
    thrust::sort(thrust::device,
        refsTBI, refsTBI + N, [outStratCount] BHD(TBITuple a, TBITuple b) {
        S32 sa = thrust::get<2>(a); // Segment index in output arrays
        S32 sb = thrust::get<2>(b);

        int dim = (stratDimMask & outStratCount[sa]) >> stratBitOffset;
        S32 la = outStratCount[sa]; // sort by strategy and segment index in reference arrays (only strategy is relevant so far)
        S32 lb = outStratCount[sb];

        F32 ca = thrust::get<1>(a).min()[dim] + thrust::get<1>(a).max()[dim]; // centroid in dim
        F32 cb = thrust::get<1>(b).min()[dim] + thrust::get<1>(b).max()[dim];
        // return (la < lb) || (la == lb && ((sa < sb) || (sa == sb && (ca < cb || (ca == cb && thrust::get<0>(a) < thrust::get<0>(b))))));
    ...
    thrust::for_each(thrust::device, thrust::make_counting_iterator((S32)0), thrust::make_counting_iterator((S32)N),
        ...
        if (leftIdx[i] >= outIdxBest[s])
            // My offset within segment is to the right of the split index
            keys[i] = keys[i] | 1ull << (U64)(63 - level);

        // If I'm at the start or end of the range and the gamma slot hasn't been claimed yet I record the relative split location.
        if (leftIdx[i] == 0 && gamma[i] == FW_S32_MIN)
            gamma[i] = outIdxBest[s];
        if (rightIdx[i] == 1 && gamma[i] == FW_S32_MIN)
            gamma[i] = outIdxBest[s] - leftIdx[i]; // The (negative) offset from i to the relative split location.
    });
