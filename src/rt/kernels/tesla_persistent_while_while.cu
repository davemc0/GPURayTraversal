// Modified by DaveMc to illustrate one child per lane
...
//          while (nodeAddr >= 0 && nodeAddr != EntrypointSentinel)
            while (unsigned int(nodeAddr) < unsigned int(EntrypointSentinel))   // functionally equivalent, but faster
            {
                ...
                const float4 tmp = tex1Dfetch(t_nodesA, nodeAddr + 1); // (c0.lo.z, c0.hi.z, c1.lo.z, c1.hi.z)
                      float2 n0z = *(float2*)&tmp; 
                      int2  cnodes = *(int2*)(&tmp)+1; // child_index0, child_index1
                      ...
                      const float c0loz = n0z.x  * idirz - oodz;
                      const float c0hiz = n0z.y  * idirz - oodz;
                      ...
                      // Child was not intersected => pop stack.
