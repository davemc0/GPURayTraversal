...
struct BoundsToCost
{
    BHD FIITuple operator()(BIBITuple x) // rightBounds, rightIdx, leftBounds, leftIdx
    {
        ...
        AABB nodeBounds = get<0>(x) + get<2>(x);
...
void FW::BatchSplitBVHBuilder::doGeneration(S32& N, S32& nSegments, S32 level)
{
    ...
    for (int dim = 0; dim < 3; dim++) {
        ...
        thrust::sort(thrust::device,
            ...
            S32 la = stratMask & segStratRefIdx[sa]; // sort by strategy
            S32 lb = stratMask & segStratRefIdx[sb];
        ...
        thrust::inclusive_scan_by_key(thrust::device,
            ...
            [] BHD(BITuple a, BITuple b) { return thrust::make_tuple(get<0>(a) + get<0>(b), get<1>(a) + get<1>(b)); });
        ...
        if (dim == 0) {
            ...
            thrust::transform_exclusive_scan(thrust::device,
                ...
                segStratRefIdx, [] BHD(auto v) { return get<0>(v) + get<1>(v); },
    ...
    // Make refSegIdx be the per-reference index into seg*
    ...
    cudaDeviceSynchronize(); // XXX
    for (int i = 0; i < N; i++)
        printf("post-sort i=%d refSegIdx[i]=%d refLeftIdx[i]=%d refKeys[i]=%016llx\n", i, refSegIdx[i], refLeftIdx[i], refKeys[i]);
    ...
    thrust::for_each(thrust::device, thrust::make_counting_iterator((S32)0), thrust::make_counting_iterator((S32)N),
        ...
        S32 e = (s < nSegments ? (stratNumMask & segStratRefIdx[s + 1]) : N) - 1; // OPT: Add a fake segment whose index is N to simplify this
